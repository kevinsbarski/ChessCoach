 ...
 * Detects book moves using local Polyglot opening book file
 *
 * Uses gm2600.bin (~340KB) containing ~15,000 GM-level positions
 *
 * Polyglot format: 16-byte entries
 * - 8 bytes: Zobrist hash (position identifier)
 * - 2 bytes: Move in binary format
 * - 2 bytes: Weight
 * - 4 bytes: Learn field
 */
...
const BOOK_PATH = path.join(__dirname, '../../data/gm2600.bin');
...
/**
 * The 781 Polyglot random numbers for Zobrist hashing
 * From python-chess / official Polyglot specification
 */
const RANDOM64: bigint[] = [
  ...
  0x55B6344CF97AE7E7n, 0xB862225B055B6960n, 0xCAC09AFBDDD2CDB4n, 0xDAF8E9829FE96B5Fn,
...
/**
 * Convert a FEN position to a Polyglot hash key
 */
function fenToPolyglotKey(fen: string): bigint {
  ...
  // Parse board - FEN goes from rank 8 (top) to rank 1 (bottom)
  ...
  for (let rank = 7; rank >= 0; rank--) {
    ...
    for (const char of row) {
      if (char >= '1' && char <= '8') {
        ...
        const pieceIdx = PIECE_TO_POLYGLOT[char];
        if (pieceIdx !== undefined) {
          // Polyglot index: piece_type * 64 + rank * 8 + file
          const randIdx = pieceIdx * 64 + rank * 8 + file;
          key ^= RANDOM64[randIdx];
        }
        file++;
      }
    }
  ...
  // En passant - only include if there's actually a pawn that can capture
  // This matches Polyglot book generation behavior
  if (enPassant !== '-') {
    const epFile = enPassant.charCodeAt(0) - 'a'.charCodeAt(0);
    const epRank = parseInt(enPassant[1]) - 1;

    // Determine the rank where an enemy pawn would need to be to capture
    // If en passant is on rank 3 (after 1.e4), we need a black pawn on rank 4 (index 3)
    // If en passant is on rank 6 (after 1...e5), we need a white pawn on rank 5 (index 4)
    const pawnRank = epRank === 2 ? 3 : 4; // rank 3 → check rank 4, rank 6 → check rank 5
    const enemyPawn = turn === 'w' ? 'p' : 'P'; // Looking for enemy pawns

    // Check if there's an enemy pawn that can actually capture
    let canCapture = false;
    const boardRows = board.split('/');

    // Get the row where enemy pawns would be (FEN rows are rank 8 down to rank 1)
    const fenRowIndex = 7 - pawnRank;
    const pawnRow = boardRows[fenRowIndex];

    // Parse the row to find pawns at adjacent files
    let currentFile = 0;
    for (const char of pawnRow) {
      if (char >= '1' && char <= '8') {
        currentFile += parseInt(char);
      } else {
        // Check if this is an enemy pawn on an adjacent file
        if (char === enemyPawn) {
          if (currentFile === epFile - 1 || currentFile === epFile + 1) {
            canCapture = true;
            break;
          }
        }
...
/**
 * Decode a Polyglot move (2 bytes) to UCI format
 */
function decodePolyglotMove(move: number): string {
  const toFile = move & 0x07;
  const toRank = (move >> 3) & 0x07;
  const fromFile = (move >> 6) & 0x07;
  const fromRank = (move >> 9) & 0x07;
  const promotion = (move >> 12) & 0x07;
...
// Cached book entries: Map<hash, moves[]>
let bookEntries: Map<string, string[]> | null = null;

/**
 * Load the opening book into memory
 */
function loadBook(): Map<string, string[]> {
  if (bookEntries) return bookEntries;

  bookEntries = new Map();

  try {
    if (!fs.existsSync(BOOK_PATH)) {
      console.warn('Opening book file not found:', BOOK_PATH);
      return bookEntries;
    }

    const buffer = fs.readFileSync(BOOK_PATH);
    const entryCount = Math.floor(buffer.length / 16);

    for (let i = 0; i < entryCount; i++) {
      const offset = i * 16;

      // Read 8-byte hash as BigInt
      const hashHigh = buffer.readUInt32BE(offset);
      const hashLow = buffer.readUInt32BE(offset + 4);
      const hash = (BigInt(hashHigh) << 32n) | BigInt(hashLow);
      const hashStr = hash.toString(16).padStart(16, '0');

      // Read 2-byte move
      const move = buffer.readUInt16BE(offset + 8);
      const uci = decodePolyglotMove(move);

      // Store in map
      if (!bookEntries.has(hashStr)) {
        bookEntries.set(hashStr, []);
      }
      bookEntries.get(hashStr)!.push(uci);
    }

    console.log(`Loaded ${entryCount} book entries from ${BOOK_PATH}`);
  } catch (error) {
...
// Simple in-memory cache for FEN lookups
const fenCache = new Map<string, string[]>();

/**
 * Get all book moves for a FEN position
 */
function getBookMoves(fen: string): string[] {
  if (fenCache.has(fen)) {
    return fenCache.get(fen)!;
  }

  const book = loadBook();
  const key = fenToPolyglotKey(fen);
  const hashStr = key.toString(16).padStart(16, '0');
  const moves = book.get(hashStr) || [];
 ...
 * @returns true if the position has book moves available
 */
export async function isBookPosition(fen: string): Promise<boolean> {
  const moves = getBookMoves(fen);
 ...
 * @returns true if this move exists in the opening book for this position
 */
export async function isBookMove(fen: string, move: string): Promise<boolean> {
  const moves = getBookMoves(fen);
  const normalizedMove = move.toLowerCase().replace(/[x+#=]/g, '');
  return moves.some(m => m === normalizedMove || m.startsWith(normalizedMove));
}

/**
 * Rate limit delay - kept for API compatibility but now instant (no-op)
 * Local Polyglot lookups are fast (<1ms) so no delay needed
 */
